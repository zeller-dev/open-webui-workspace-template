...
- Published port (`11434` by default) still works for inbound host access
  even though the network is internal — that's normal Docker behavior,
  internal only blocks the container's own outbound traffic.